...
def print_menu(stdscr, selected_row_idx):
    description_table = [
        'First Description',
        'Lorem Ipsum and some longer description to show overflow possibly.',
    ...
    titleBar.addstr(2,2, "SELECT A FILE TO HEAR MORE MORLOFF")
    titleBar.border(0)
    ...
    for idx, row in enumerate(menu):
        ...
        if idx == selected_row_idx:
            ...
            for j, text in enumerate(textwrap.wrap(description_table[idx], w * 2//3 - 5 )):
                descriptionWindow.addstr(j + 3, 2, text)
                # descriptionWindow.addstr(3, 1, textwrap.wrap(description_table[idx], 10))
            # stdscr.addstr((h // 2 - len(menu) // 2), x + 25, textwrap.fill(description_table[idx]), 10)
        else:
